...
上面已经提到了动态添加Components的能力，那么添加的这些Components选用什么基类呢？ModularGameplay同样贴心的提供了好用的GameFrameworkComponent系列Components, 这些Components提供了很多好用的帮助函数，比如GetGameInstance, GetGameState等等，而且可以跟ModularGameplayActors中的Actors有很多的互动，比如AModularGameState在HandleMatchHasStarted函数会尝试获取身上所有的UGameStateComponent并调用其HandleMatchHasStarted函数。GameFrameworkComponent提供了这些即插即用的Components：

* ControllerComponent:  用于添加到Controller上的组件
* GameStateComponent:   用于添加到GameState上的组件
* PawnComponent:        用于添加到Pawn上的组件
* PlayerStateComponent: 用于添加到PlayerState上的组件

ModularGameplayActors和GameFrameworkComponent搭配使用效果最佳。
...
LyraPawnExtensionComponent它是初始化系统的总指挥，它从来不依赖其他Components的States, 其他的Components都要直接或者间接监听它的状态，比方说很重要的CharacterInited这个时刻，LyraPawnExtensionComponent需要查询绑定在本Actor的所有Components是否都GameplayReady了。

LyraPawnExtensionComponent这个组件特别重要，外部其他Gameplay的系统都知道Character身上LyraPawnExtensionComponent这个组件的存在(定义在Character的基类中)，那些动态生成的Components以及需要InitStates功能的Components也指定LyraPawnExtensionComponent的存在，而LyraPawnExtensionComponent不知道其他Components的存在，因此可以看到LyraPawnExtensionComponent代码中没有假定某个Component存在的代码，LyraPawnExtensionComponent充当了“中间人”的角色。

![LyraPawnExtensionComponent](./UE5ModularGameplayPic/LyraPawnExtensionComponent.png)

LyraPawnExtensionComponent在实现的几个要点：
...
3. CanChangeInitState中允许从InitState_DataAvailable过渡到InitState_DataInitialized的条件是所有Components都到达了InitState_DataAvailable，这行代码充分体现了ULyraPawnExtensionComponent统筹管理的角色

4. 调用BindOnActorInitStateChanged并且FeatureName为空，这样当其他的Components状态更新时都会调用ULyraPawnExtensionComponent::OnActorInitStateChanged，这对于ULyraPawnExtensionComponent能够统筹管理来讲特别重要

5. 定义一些Gameplay所需的代理，比如OnAbilitySystemInitialized，OnPawnGameplayReady等等，原因在于LyraPawnExtensionComponent“中间人”和统筹管理的角色

再次回到Lyra上，我们通过下图看下LyraHeroComponent是如何通过ULyraPawnExtensionComponent完成初始化的：

![PawnExtensionAndHero](./UE5ModularGameplayPic/PawnExtensionAndHero.png)

1. 两个组件调用BeginPlay后会尝试进入到Spawned状态
2. 两个组件都尝试进入到下一个状态DataAvailable, 但是数据都准备好，因此不会切换
3. 当前很多事件发生时都会调用LyraPawnExtensionComponent的接口，每次都会尝试进入到下一状态
4. 终于在某一个时刻，数据都准备好了，两个组件都进入到了下一个状态DataAvailable
5. 因为LyraPawnExtensionComponent调用过BindOnActorInitStateChanged并且FeatureName为空，因此所有Components状态切换时都会监测到，OnActorInitStateChanged函数则会继续尝试状态切换
6. LyraPawnExtensionComponent检测到了所有的Components都进入到了DataAvailable, 自己成功切换到了DataInitialized状态
7. LyraHeroComponent有LyraPawnExtensionComponent到达DataInitialized的回调，另一方面LyraPawnExtensionComponent进入到了GameplayReady
8. LyraPawnExtensionComponent到达DataInitialized的回调函数会继续尝试状态切换，LyraHeroComponent成功切换到DataInitialized, 这时候所有的数据都OK了，包括PawnData(LyraPawnExtensionComponent在DataAvailable状态就意味着PawnData已经可以去获取了)，这个时刻可以去初始化各种系统了
9. 继续尝试状态切换，成功切换到GameplayReady

>_PawnExtendionComponent不适合什么回调都往里放，不仅仅越来越臃肿，而且回调发生后会通知所有实现者都check, 一些完全不在乎这个数据的实现者也不得不check,PawnExtensionComponent适合一些基础数据的check,比如PlayerController和PlayerState, 相关Components可以通过事件系统，全局回调等去检测自己所需的_

>_警告: 多个Components调用CheckDefaultInitializationForImplementers会进入死循环,只有LyraPawnExtensionComponent调用就可以了_

#### 状态机更新经典代码

利用栈做代理函数的调用，避免递归调用

UGameFrameworkComponentManager::ProcessFeatureStateChange

## 总结
* GameFeatures, Experience，GameFeatureAction, ModularGameplay都是为模块化玩法设计的

* 如果想使用ModularGameplay, ModularGameplayActors和GameFrameworkComponent也应该默认开启使用的

* LyraPawnExtensionComponent可以直接用于项目

* 即使不用模块化玩法，InitStates的功能也可以使用

* 想要自定义GameFeatureAction的话，必须得依靠ExtensionHandlerSystem

* 自定义GameFeatureAction实现添加逻辑时，一定要保证函数重复调用不报错并且只生效一次，比如AddWidget, 不应该出现显示两个UI

* DataAvailable表示自己所需的数据都有效了，DataInitialized表示利用现有的数据完成了自己内部数据的初始化
