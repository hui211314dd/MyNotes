...
纹理映射的基本原理如下图所示，对于每个像素都有对应的uv坐标，通过uv坐标可以从纹理图中获取到颜色值，将这个颜色值替换到上面布林冯模型的Kd系数上即可，有个好处是应用贴图的同时也有光照反射效果。

![纹理映射基本原理](./Game101Pic/纹理映射基本原理.png)

纹理映射常常会出现一些问题：

### 纹理太小

比如一堵4K分辨率的墙需要贴一张256的贴图，墙上的一个Pixel要对应纹理图上的一个Texel, 如果按照取整的方式选取Texel的话，那么会出现墙上多个点对应的是同一个Texel.

![Nearest的方式](./Game101Pic/Nearest的方式.png)

下图可以看到，当UV采样到任一红点位置时返回的都是同一个Texel的颜色值，因此可以看到颜色成块状；

![Nearest的原理](./Game101Pic/Nearest的原理.png)

Nearest：当UV是非整数时，那么会找最近的整数并取值；

![Bilinear的方式](./Game101Pic/Bilinear的方式.png)

Bilinear: 查找临近的四个Texel，按照位置线性插值；

### 纹理太大

![纹理过大导致的问题](./Game101Pic/纹理过大导致的问题.png)

原因在于远处一个像素可能会覆盖很多个Texels，如果单纯的取一个Texel的颜色值，结果必然是不对的。

![远处的一个Pixel会覆盖多个Texels](./Game101Pic/远处的一个Pixel会覆盖多个Texels.png)


依照以往的经验如果超采样的话，比如一个像素点分成512个采样点，会得到一个理想的结果，但是随之而来的消耗也是巨大的。

![512超采样](./Game101Pic/512超采样.png)

我们回顾一下走样产生的原因：**信号变化过快，采样频率/速度跟不上**，那么纹理过大产生的走样如何用信号采样解释呢？当纹理特别大时，一个Pixel可能包含很大一块纹理，这一大块纹理一直在变化，而只用一个采样点肯定是不行的，为了解决必须得使用更高频的采样方法；512个采样点就是提供了更高频的采样把结果综合起来；

更高频的采样消耗巨大，我们换一种思路，不采样而是直接查询这个区域的平均值是多少，如果可以直接查询某个区域的平均值的话，这个问题就可以解决了。

![点查询和范围查询](./Game101Pic/点查询和范围查询.png)

这就引出了Mipmap的概念。

### Mipmap

Mipmap允许我们做**快速，近似，正方形区域**的范围查询

Mipmap可以一张图生成一系列图：

![Mipmap生成一系列图](./Game101Pic/Mipmap生成一系列图.PNG)

生成的Mipmap一系列图存储量增加了1/3.

随之而来的问题是**如何知道这个Pixel映射的正方形区域是什么**以及**这个正方形区域对应的颜色是什么**

1. 如何知道这个Pixel映射的正方形区域是什么
   
   屏幕空间上任何一个像素都能映射到纹理上一个点, 我们再查询这个像素右边和上边像素对应的纹理点，就能大概算出这个像素点大概覆盖纹理的范围。

   ![如何知道这个Pixel映射的正方形区域是什么](./Game101Pic/如何知道这个Pixel映射的正方形区域是什么.PNG)

2. 这个正方形区域对应的颜色是什么
   
   通过第一步我们得到了L, 通过公式D=LogL可以知道应该从第几层查询到这个点的颜色，D表示在第D层上这个L区间会变成一个像素；但是大部分情况得到的D不一定是整数，可能是4.8，那么分别在Level4和Level5上分别采样，最后在两个层直接在做插值，这成为三线性插值；

   ![三线性插值](./Game101Pic/三线性插值.PNG)

   ![D直接取整](./Game101Pic/D直接取整.PNG)

   ![三线性插值结果对比](./Game101Pic/三线性插值结果对比.PNG)

### Mipmap的问题

继续我们上面的纹理映射，如果应用Mipmap后可以看到如下结果：

![Mipmap带来的问题](./Game101Pic/Mipmap带来的问题.PNG)

可以看到有些模糊过度了，为什么呢？Mipmap的特点是必须是查询**正方形区域**，而上图很多时候需要查询的都是一些**矩形区域**，因为得到的结果很不准确，有另外一个方法可以部分解决这个问题，那就是Anisotropic Filtering（各向异性过滤），它实际上支持矩形区域的范围查询：

![各向异性过滤的结果](./Game101Pic/各向异性过滤的结果.PNG)

各向异性过滤生成的图还包括了一些被压扁的图，这样的话支持查询矩形区域的平均值，但可以看到所需的存储量也增加了3倍！

![各向异性过滤生成的图](./Game101Pic/各向异性过滤生成的图.PNG)

下图可以看到各向异性过滤仅仅解决了部分问题，标有×的矩形是一个斜着的，即使使用各向异性过滤得到的结果任然是Overblur的。

![各向异性过滤解决了部分问题](./Game101Pic/各向异性过滤解决了部分问题.PNG)

一些游戏设置中经常出现各向异性过滤2X, 4X，就是表示生成图的层数。

![各向异性过滤的层数](./Game101Pic/各向异性过滤的层数.PNG)
